...
void write(vec4 v) { write(v.x, v.y, v.z, v.w); }
...
struct mat4 {
  real a[] = new real[16];
  real val(int row, int col) { return a[row * 4 + col]; }
  void set(int row, int col, real val) { a[row * 4 + col] = val; }
};
mat4 make_mat4() {
  mat4 m;
  for (int i = 0; i < 16; ++i)
...
tri make_tri(vec4 a, pair tex_coord_a, vec4 b, pair tex_coord_b, vec4 c,
             pair tex_coord_c) {
  tri t;
  t.a.pos = a;
  t.a.tc = tex_coord_a;
...
// (Diese Funktion wird weiter unten definiert und soll von Ihnen
  ...
  for (int y = y_min; y <= y_max; y += 1)
    for (int x = x_min; x <= x_max; x += 1) {
      ...
      bool point_in_tri(real pixel_x, real pixel_y) {
        if (dot(n_a, (pixel_x, pixel_y) - assume_2d(tri.a.pos)) < 0)
          return false;
        if (dot(n_b, (pixel_x, pixel_y) - assume_2d(tri.b.pos)) < 0)
          return false;
        if (dot(n_c, (pixel_x, pixel_y) - assume_2d(tri.c.pos)) < 0)
          return false;
        return true;
      }
      real pixel_x = x + 0.5;
...
//
// Berechnung der baryzentrischen Koordinate eines Punkts p bezüglich eines
// Dreiecks.
//
...
if (mode == "head-on") {
  t1 = make_tri(make_vec(-3, -3, -10), (0, 0), make_vec(3, -3, -10),
                (tc_max, 0), make_vec(3, 3, -10), (tc_max, tc_max));

  t2 = make_tri(make_vec(-3, -3, -10), (0, 0), make_vec(3, 3, -10),
                (tc_max, tc_max), make_vec(-3, 3, -10), (0, tc_max));
} else if (mode == "slant") {
  t1 = make_tri(make_vec(-3, -3, -10), (0, 0), make_vec(3, -3, -15),
                (tc_max, 0), make_vec(3, 3, -15), (tc_max, tc_max));

  t2 = make_tri(make_vec(-3, -3, -10), (0, 0), make_vec(3, 3, -15),
                (tc_max, tc_max), make_vec(-3, 3, -10), (0, tc_max));
} else if (mode == "SLANT") {
  t1 = make_tri(make_vec(-3, -3, -10), (0, 0), make_vec(3, -3, -25),
                (tc_max, 0), make_vec(3, 3, -25), (tc_max, tc_max));

  t2 = make_tri(make_vec(-3, -3, -10), (0, 0), make_vec(3, 3, -25),
                (tc_max, tc_max), make_vec(-3, 3, -10), (0, tc_max));
}
