...
IMPORT INTO ST_META_POLITBAROMETER_COLUMN_LABELS (variable_id, label)
    FROM LOCAL CSV FILE '/tmp/st_meta_politbarometer_column_labels.csv' IGNORE CERTIFICATE ERRORS INTO error_tbl REJECT LIMIT UNLIMITED;

IMPORT INTO ST_META_POLITBAROMETER_VALUE_LABELS (VARIABLE_ID, VALUE_ID, LABEL)
    FROM LOCAL CSV FILE '/tmp/st_meta_politbarometer_value_labels.csv' IGNORE CERTIFICATE ERRORS INTO error_tbl REJECT LIMIT UNLIMITED;

IMPORT INTO ST_META_BUNDESTAG_PARTIES (SHORTNAME, NAME)
    FROM LOCAL CSV FILE '/tmp/parteien.tsv'
    COLUMN SEPARATOR = 'TAB' SKIP =1 IGNORE CERTIFICATE ERRORS INTO error_tbl REJECT LIMIT UNLIMITED;

IMPORT INTO ST_META_BUNDESTAG_DISTRICTS (DISTRICT_ID, DISTRICT_NAME, STATE_NAME)
    FROM LOCAL CSV FILE '/tmp/st_meta_bundestag_districts.csv'
    (1, 2, 8)
    COLUMN SEPARATOR = ';' SKIP = 8 IGNORE CERTIFICATE ERRORS INTO error_tbl REJECT LIMIT UNLIMITED;
